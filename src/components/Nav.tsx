	...
	return (
		<div className="absolute top-0 right-0 left-0 w-full my-2 px-2 flex justify-between bg-black bg-opacity-20 backdrop-blur-lg rounded-lg z-50">
			<div onClick={() => navigate("/")}>
				<img className="w-20" src={marvelIcon} />
			</div>
			<button className="w-20" onClick={() => dispatch(switchTheme())}>
				{isLightMode ? (
					<MdOutlineDarkMode className="w-full h-8 text-black" />
				) : (
					<MdOutlineLightMode className="w-full h-8" />
				)}
			</button>
		</div>
	);
